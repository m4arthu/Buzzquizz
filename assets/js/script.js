...
function BuscarQuizz(quizid){
    ...
    let promess =axios.get(`https://mock-api.driven.com.br/api/vm/buzzquizz/quizzes/${quizid}`);
...
function geraropcoes(idpergunta){
    ...
    for (let i = 0; i < respostas.length; i++) {
        ...
        if (respostas[i].isCorrectAnswer) {
            Opcoes.innerHTML += `
            <div class="cardQuizz opcao correta" onclick="selecionarOpcao(this)">
        ...
        `;
            ...
            Opcoes.innerHTML += `
            <div class="cardQuizz opcao" onclick="selecionarOpcao(this)">
                <img src="${respostas[i].image}">
                <p>${respostas[i].text}</p>
            </div>
        `;
...
function selecionarOpcao(){
   ...
   // criar função de selecionar resposta
}
...
function renderizarQuizCriado(quiz) {
    quiz = axios
        .get('https://mock-api.driven.com.br/api/vm/buzzquizz/quizzes/356')
        .then(Quiz => {
...
function restartQuizz() {
    ...
    const btnRestart = document.querySelector('.restart-quizz')
    window.scrollTo({
        top: 0,
        behavior: 'smooth'
    })
